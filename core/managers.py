    ...
    if image.rgb_views.count() == 0:
        ...
        if image.store.c_size == 3:
            rgb_context = models.RGBRenderContext.objects.create(
                image=image, name="RGB", description="Default RGB Context"
            )

            red_view = models.RGBView.objects.create(
                image=image, color_map=enums.ColorMapChoices.RED, c_min=0, c_max=1
            )
            green_view = models.RGBView.objects.create(
                image=image, color_map=enums.ColorMapChoices.GREEN, c_min=1, c_max=2
            )
            blue_view = models.RGBView.objects.create(
                image=image, color_map=enums.ColorMapChoices.BLUE, c_min=2, c_max=3
            )
