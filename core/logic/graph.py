...
def create_axes(system: "models.CoordinateSystem", axes: list, *, as_array_indices: bool = False) -> list["models.Axis"]:
    """Write a coordinate system's axes, enumerating them so `order` is the array index.
    ...
    With ``as_array_indices``, the axes keep their names and order but become raw
    voxel indices -- discrete, unitless, typed ARRAY. That is what an ARRAY system
    (a pyramid level, or a lens) has.
    """
    rows = []
    for index, axis in enumerate(axes):
        axis_type = axis.type.value if hasattr(axis.type, "value") else axis.type
        unit = None if as_array_indices else axis.unit
        if unit is not None:
            # A unit pint cannot parse is worthless: it will fail at the moment
            # someone tries to convert with it, which is long after the write and
            # far from whoever made it. Reject it here, so a direct ORM write is
            # held to the same standard as the GraphQL boundary.
            unit = kanne_scalars.parse_unit(unit)
        rows.append(
            models.Axis(
                coordinate_system=system,
                order=index,
                name=axis.name,
                type=enums.AxisTypeChoices.ARRAY.value if as_array_indices else axis_type,
                unit=unit,
                discrete=True if as_array_indices else axis.discrete,
...
def create_level_edge(
    *,
    ...
    base_spacing: list[float],
    shape_0: list[int],
    shape_level: list[int],
    axis_specs: list[coords_logic.AxisSpec],
    ctx: CreationContext,
) -> "models.Transformation":
    """Store the edge placing one pyramid level in its dataset's intrinsic space.

    The scale is **absolute**, derived from the actual shapes -- so a pyramid whose
    axes do not halve cleanly (36 floors to 18, 9, 4, 2, 1, giving factors 1, 2, 4,
    **9, 18, 36**, not 1, 2, 4, 8, 16, 32) is described correctly rather than
    plausibly. The translation is the half-voxel offset the downsample introduces;
    without it every level above 0 draws offset from level 0.
    ...
    """
    scale, translation = coords_logic.pyramid_transform(base_spacing, shape_0, shape_level, axis_specs)
...
def path_to_intrinsic(system: "models.CoordinateSystem") -> list[tuple[str, dict]]:
    """The chain of edges from a system up to its dataset's intrinsic space.

    Scene-independent and always defined, which is exactly why the ROI bounding box
    is expressed against it rather than against a scene's world: world is
    scene-owned, and one dataset can sit in two scenes under two registrations.
...
def compute_intrinsic_bbox(system: "models.CoordinateSystem", vectors: list[list[float]]) -> dict | None:
    ...
    try:
        ...
        # An ATLAS or world system has no intrinsic space to reach. The box is
        # still meaningful in the system's own coordinates.
        chain = []
