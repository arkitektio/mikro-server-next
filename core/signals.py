...
def my_snapshot_handler(sender, instance=None, created=None, **kwargs):
    if created:
        image_broadcast(
            {"id": instance.image.id, "type": "update"},
...
def my_image_handler(sender, instance=None, created=None, **kwargs):
    image_broadcast(
        {"id": instance.id, "type": "create" if created else "update"},
        ["images"] + [f"dataset_images_{instance.dataset.id}"],
    )
...
def my_file_handler(sender, instance=None, created=None, **kwargs):
    ...
    file_broadcast(
        {"id": instance.id, "type": "create" if created else "update"}, channels
    )
